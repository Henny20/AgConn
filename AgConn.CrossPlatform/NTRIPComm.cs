...
//using AgConn.CrossPlatform.Dialogs;
//using AAgIO.Views;
...
namespace AgConn.CrossPlatform
{
    public partial class AgConnService
    {
        ...
        public string broadCasterIP {get; set;}
        ...
        public bool isNTRIP_RequiredOn {get; set;} = false;
        ...
        public bool isRadio_RequiredOn {get; set;} = false;
        public bool isSerialPass_RequiredOn {get; set;} = false;
        ...
        private void DoNTRIPSecondRoutine()
        {
            ...
            if (isNTRIP_Connecting)
            {
                if (ntripCounter > 29)
                {
                    //TODO var tmb = new TimedMessageBox(2000, "Connection Problem", "Not Connecting To Caster");
                    //TODO  tmb.ShowDialog(this);
                    Console.WriteLine("Connection Problem" + "  Not Connecting To Caster");
                    ReconnectRequest();
                }
            ...
            if (isNTRIP_RequiredOn || isRadio_RequiredOn)
            {
                ...
                //// TODO lblNTRIPBytes.Text = ((tripBytes >> 10)).ToString("###,###,### kb"); No Click
                // TODO lblNTRIPBytes.Content = ((tripBytes >> 10)).ToString("###,###,### kb"); 

                //Bypass if sleeping
                if (focusSkipCounter != 0)
                {
                    //update byte counter and up counter
                    if (ntripCounter > 59) Console.WriteLine("TODO"); //btnStartStopNtrip.Content = (ntripCounter >> 6) + " Min";
                    else if (ntripCounter < 60 && ntripCounter > 22) Console.WriteLine("TODO"); //btnStartStopNtrip.Content = ntripCounter + " Secs";
                    else Console.WriteLine("TODO");//btnStartStopNtrip.Content = "In " + (Math.Abs(ntripCounter - 22)) + " secs";

                    //watchdog for Ntrip
                    if (isNTRIP_Connecting)
                    {
                        // TODO lblWatch.Text = gStr.gsAuthourizing;
                    }
                    else
                    {
                        if (isNTRIP_RequiredOn && NTRIP_Watchdog > 10)
                        {
                            // TODO lblWatch.Text = gStr.gsWaiting;
                        }
                        else
                        {
                            // TODO lblWatch.Text = gStr.gsListening;

                            if (isNTRIP_RequiredOn)
                            {
                                // TODO lblWatch.Text += " NTRIP";
                            }
                            else if (isRadio_RequiredOn)
                            {
                                // TODO lblWatch.Text += " Radio";
                            }
                        }
                    }
                    ...
                    {
                        // TODO lblWatch.Text = "Send GGA";
                        Console.WriteLine("Send GGA");
            ...
            {
                //pbarNtripMenu.Value = unchecked((byte)(tripBytes * 0.02));
                // TODO lblNTRIPBytes.Content = ((tripBytes >> 10)).ToString("###,###,### kb"); //No Click
        ...
        {
            // TODO lblWatch.Text = "Wait GPS";
            // TODO lblMessages.Content = "Reading...";
            // TODO lblNTRIP_IP.Text = "";
            // TODO lblMount.Text = "";
            ...
            if (isNTRIP_RequiredOn || isRadio_RequiredOn || isSerialPass_RequiredOn)
            {
                // TODO btnStartStopNtrip.IsVisible = true;
                //TODO  btnStartStopNtrip.IsVisible = true;
                // TODO lblWatch.IsVisible = true;
                // TODO lblNTRIPBytes.IsVisible = true;
                // TODO lblToGPS.IsVisible = true;
                // TODO lblMount.IsVisible = true;
                // TODO lblNTRIP_IP.IsVisible = true;
            }
            else
            {
                // TODO  btnStartStopNtrip.IsVisible = false;
                //TODO btnStartStopNtrip.IsVisible = false;
                // TODO lblWatch.IsVisible = false;
                // TODO lblNTRIPBytes.IsVisible = false;
                // TODO lblToGPS.IsVisible = false;
                // TODO lblMount.IsVisible = false;
                // TODO lblNTRIP_IP.IsVisible = false;
            }

            //TODO btnStartStopNtrip.Content = "Off";
        }
        ...
        public void StartNTRIP()
        {
            if (isNTRIP_RequiredOn)
            {
                ...
                // TODO lblNTRIP_IP.Text = broadCasterIP;
                // TODO lblMount.Text = mount;
            }
            else if (isRadio_RequiredOn)
            {
                if (!string.IsNullOrEmpty(Properties.Settings.Default.setPort_portNameRadio))
                {
                    ...
                    catch (Exception ex)
                    {
                        ...
                        //TODO  var form = new TimedMessageBox(2000, "Error connecting to radio", $"{ex.Message}");
                        //TODO form.ShowDialog(this);
            ...
            else if (isSerialPass_RequiredOn)
            {
                ...
                if (!string.IsNullOrEmpty(Properties.Settings.Default.setPort_portNameRadio))
                {
                    ...
                    isNTRIP_Connecting = false;
                    isNTRIP_Connected = true;
                    // TODO lblWatch.Text = "RTCM Serial";
                    ...
                    catch (Exception ex)
                    {
                        ...
                        //TODO var form = new TimedMessageBox(2000, "Error connecting to Serial Pass", $"{ex.Message}");
                        //TODO  form.ShowDialog(this);
                        Console.WriteLine("Error connecting to Serial Pass" + ex.Message);
                    }
                }
            }
        }
        ...
        public void OnAddMessage(byte[] data)
        {
            ...
            {
                int mess = 0;
                // TODO lblPacketSize.Text = data.Length.ToString();

                try
                {
                    // TODO lblStationID.Text = (((data[4] & 15) << 8) + (data[5])).ToString();
 ...
 isNTRIP_RequiredOn = Properties.Settings.Default.setNTRIP_isOn;
Console.WriteLine("NTRIP on: " + isNTRIP_RequiredOn);
            ...
            {
                // TODO lblToGPS.Text = data.Length.ToString();
                //send it
                SendNTRIP(data);
            }
        ...
        private void ntripMeterTimer_Tick(object sender, EventArgs e)
        {
            ...
            if (rawTrip.Count == 0)
            {
                ...
                if (focusSkipCounter != 0)
                {
                    // TODO lblToGPS.Text = traffic.cntrGPSInBytes == 0 ? "--" : (traffic.cntrGPSInBytes).ToString();
            ...
            if (isViewAdvanced) 
                Console.WriteLine("Count: " + rawTrip.Count.ToString());
            // TODO lblCount.Text = rawTrip.Count.ToString();
        }
        ...
        public void SendNTRIP(byte[] data)
        {
            ...
            //send out UDP Port
            Console.WriteLine("sendto UDP" + isSendToUDP);
            if (isSendToUDP)
            {
